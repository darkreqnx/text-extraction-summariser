...
"""
set of very commonly used words blacklisted as stop words 
to prevent redundancy and noise in keyword count
"""
stopWords = set([
...
def Summarize(title, text):
    ...
    # if input text is very short (<=5 sentences), then simply return the list sentences
    if len(sentences) <= 5:
        return sentences

    # otherwise, score setences, and use the top 5 sentences
...
def keywords(text):
    """
    splits text into words, gets the top ten keywords and associated frequency scores
    creates dictionary {keyword:freqScore}
    ...
    keywords = {x: y for x, y in freq.most_common(minSize)}  # recreate a dict {key : freqScore}
